...
**画像の権利に関する注意**：本ツールは、自作画像、または利用する権利と生成サービスの利用条件を確認したAI生成画像によるオリジナル素材の制作を想定しています。第三者の既存の絵・写真・ゲーム素材等を読み込んで転用する用途は想定していません。権利者の許諾なく、法令上認められない読み込み・加工・公開・配布を行わないでください。 AI生成画像についても第三者の権利を侵害しないことを確認してください。詳しくは[免責事項](docs/DISCLAIMER.md#読み込む画像の権利と利用者の責任)を参照してください。
...
描画操作からシンプル（左描画・右消去）、多機能（左描画・右スポイト）、二刀流（左右で別色）を選べます。二刀流の右色はパレットを右クリックして指定します。多機能・二刀流では鉛筆で同じ画素を同じボタンで再クリックすると直前の色に戻ります。範囲・色・ツール変更や他の操作を挟むと、この取消情報は破棄されます。

描画モード・左右の色・編集範囲・グリッド・前フレーム表示・全体プレビュー倍率はブラウザのローカルストレージに記憶します。同じアドレスのブラウザで復元され、ブラウザデータ削除時には消えます。プロジェクトには含めません。

画像取り込みの未確定プレビューはドラッグ、または位置X/Yの数値指定で移動できます。「中央に戻す」でオフセットをリセットします。確定までは原画を変更しません。

全体編集も編集範囲の選択肢から選べます。256×192の素材なら「256×192編集（全体）」と表示し、「全体を編集」ボタンと連動します。「部品を描く」で直前の部品編集サイズに戻ります。
